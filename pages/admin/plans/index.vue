...
<div class="pb-32 content-center ">
          <h2 class="text-4xl text-center py-4">Plans</h2>
        <div class="container mx-auto px-6 overflow-x-auto">
          <div class="flex flex-row justify-between">
            <div class="">
            <span > Title</span>
     <form>
                      <input @keyup.enter="getData" v-model="filter.title" type="text" placeholder="Search" class=" placeholder:italic placeholder-slate-400 rounded mt-1 px-2 border border-slate-600  h-10    contrast-more:border-slate-400 contrast-more:placeholder-slate-500"/>
<!--front end validation for search-->
            ...
            <span > Amount</span>
     <form>
                      <input @keyup.enter="getData" v-model="filter.amount" type="text" placeholder="Search" class=" placeholder:italic placeholder-slate-400 rounded mt-1 px-2 border border-slate-600  h-10    contrast-more:border-slate-400 contrast-more:placeholder-slate-500"/>
<!--front end validation for search-->
          </form>
            </div>
                <Button
...
<div class="w-70 hidden">Date Picker </div>
  ...
  <tbody v-if="!loading">
    <tr v-for="(item, index) in planList" :key="item.id" class=" space-x-8 space-y-1 text-center text-sm">
      <td>{{index +1}}</td>
  ...
  <div class="flex-1">
    <span> Title</span>
     <form>
            <input v-model="formData.title" placeholder="Eg: 4G-55" type="text" class="border-slate-200 my-1  p-2 w-11/12 placeholder:italic placeholder-slate-400 rounded mt-1 px-2 border border-slate-600"/>
              <InputValidation :error-text="formValidator.getError('title')" />
   ...
   <div class="flex-1">
    <span> Amount</span>
     <form>
            <input v-model="formData.amount" type="text" placeholder="Eg: 55" class="border-slate-200 my-1  p-2 w-11/12 placeholder:italic placeholder-slate-400 rounded mt-1 px-2 border border-slate-600"/>
              <InputValidation :error-text="formValidator.getError('amount')" />
...
getData () {
      this.loading = true
  let geturl =`plan/`
  if(this.filter.amount || this.filter.title){
    geturl = geturl + `?title=${this.filter.title}&amount=${this.filter.amount}`
  }
    ...
    postData() {
      this.formData.title = this.formData.title.toUpperCase()
      console.log(this.formData)
      if (!this.formData.id) {
          console.log(this.formData);
          this.$axios
            .$post('plan/', this.formData)
            .then((response) => {
              this.planList.unshift(response)
              ...
              this.$store.dispatch('showSnackbar', {
                text: 'New Plan Successfully Added !',
            ...
            .catch((error) => {
              ...
              this.$store.dispatch({
                message: 'New Plan Cannot be Added.',
                color: 'red',
              })
            })

      } else {
        this.$axios.$put(`plan/${this.formData.id}/`, this.formData)
          .then(() => {
            this.showForm = false
            this.getData()
            this.formData = {}
